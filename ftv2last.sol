...
contract FamilyToken is Owned {
	...
	uint256 pendingMarketingAmt;
	uint256 pendingDonationAmt;
	uint256 pendingArbitrationAmt;
...
//		uint256 tokenAmt = (_tokenAmt <= balanceOf(address(this))) ? _tokenAmt : balanceOf(address(this));
	...
	function _distributeBUSD(uint256 LPAmt, uint256 marketingAmt, uint256 donationAmt, uint256 arbitrationAmt) private {
		if (routerDefined) {
			...
			pendingMarketingAmt = pendingMarketingAmt.add(marketingAmt);
			pendingDonationAmt = pendingDonationAmt.add(donationAmt);
			pendingArbitrationAmt = pendingArbitrationAmt.add(arbitrationAmt);
			...
			if (canSwap()) {
				...
				busdManager.transfer(marketingAddress, obtainedBUSD.mul(pendingMarketingAmt).div(toSwap));
				busdManager.transfer(donationAddress, obtainedBUSD.mul(pendingDonationAmt).div(toSwap));
				busdManager.transfer(arbitrationAddress, obtainedBUSD.mul(pendingArbitrationAmt).div(toSwap));
				
				pendingMarketingAmt = 0;
				pendingDonationAmt = 0;
				pendingArbitrationAmt = 0;
